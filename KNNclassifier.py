...
def kNearesNeighbors(training_feature_vector, testInstance, k):
    distances =[]
...
def main(training_data, test_data): #색을 구별하기위해 training.data와 test.data값을 사용
    training_feature_vector = []
    test_feature_vector = []
    #training.data값과 test.data값을 test_feature_vector,training_feature_vector으로 가공
    ...
    for x in range(len(test_feature_vector)):
        #training.data와 .test.data와 거리 차이를 구함
        neighbors = kNearesNeighbors(training_feature_vector, test_feature_vector[x], k)
        #구한 거리차이로 가장 가까운 결과를 찾아냄
